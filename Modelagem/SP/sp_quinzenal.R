...
# Carrega os dados
file_path <- 'C:/Users/HP/Desktop/Projeto/Data/PreparedData/Interpolado/SP/Quinzenal/SP_CONSTRUCAO_CIVIL.csv'  
...
file_path <- 'C:/Users/HP/Desktop/Projeto/Data/PreparedData/Interpolado/SP/Quinzenal/SP_FERRAGENS.csv'  
fi <- read.csv(file_path)
fi$Proj_Date <- as.Date(fi$Proj_Date)
head(fi)
first_date <- min(fi$Proj_Date)  
...
file_path <- 'C:/Users/HP/Desktop/Projeto/Data/PreparedData/Interpolado/SP/Quinzenal/SP_MATERIAIS_ELETRICOS.csv'  
fi <- read.csv(file_path)
fi$Proj_Date <- as.Date(fi$Proj_Date)
head(fi)
first_date <- min(fi$Proj_Date)  
...
ets <- ets(timeseries, model="ZZZ")
summary(ets)

m2 <- auto.arima(timeseries)
summary(m2)
